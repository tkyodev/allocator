...
2024.01.07 Renamed to `layout_new` to make sure we are using the vendored version.

-}
layout_new : List (Attribute msg) -> Element msg -> Html msg
layout_new =
    layoutWith_new { options = [] }


{-| 2024.01.07 Renamed to `layoutWith_new` to make sure we are using the vendored version.
-}
layoutWith_new : { options : List Option } -> List (Attribute msg) -> Element msg -> Html msg
layoutWith_new { options } attrs child =
